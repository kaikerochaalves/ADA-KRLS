...
l_M = [50, 100, 150, 200, 300]
l_nu = [0.01, 0.1, 0.5]
l_sigma = [0.01, 0.05, 0.1, 0.5, 1, 5, 10, 100]
l_tau = [1e-6, 1e-4, 1e-2]
...
columns = ['M', 'nu', 'sigma', 'tau', 'RMSE', 'NDEI', 'MAE']
    ...
    for nu in l_nu:
        for sigma in l_sigma:
            for tau in l_tau:
                ...
                print(f"M = {M} and nu = {nu} and sigma = {sigma} and tau = {tau}")
            
                # Initializing the model
                model = ADA_KRLS(M = M, nu = nu, sigma = sigma, tau = tau)
                # Train the model
                ...
                NewRow = pd.DataFrame([[M, nu, sigma, tau, RMSE, NDEI, MAE]], columns = columns)
